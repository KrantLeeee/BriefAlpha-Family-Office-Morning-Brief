...
→ 用户输入 local_qa 追问
→ 后端取当前 judgement / evidence scope 的全量 evidence，做 alias + 白名单过滤
...
#### 2.6.1 Evidence QA 上下文选择

- local_evidence_qa（P0）不以全文检索作为门槛。用户从 judgement drawer 发起追问时，当前 drawer 已经定义了最小可引用范围，后端必须直接取该 judgement 展示的全部 evidence_ids（含 supplementary_sources）作为 LLM 上下文；`scope=evidence` 直接取单条 evidence。
- local scope 的上下文仍必须经过 data anonymization：只允许 `AliasedEvidence` 白名单字段进入 LLM，禁止直接拼接 FTS 行、原始持仓、客户身份或真实 ticker/company。
- local scope 支持广义追问与精确追问：例如“某个标的隔夜估算怎么算”“这些证据里几个涨几个跌”“这条证据说明什么”。不得因为用户问题未命中关键词而短路为 insufficient evidence。
- global_evidence_qa（P1）才使用 SQLite FTS5 / BM25 或等价轻量检索，从全部 `evidence_pool_full` 中召回相关 evidence；检索对象是当日 evidence chunks，而不是全互联网。
- global scope 检索无结果时不调用 LLM，直接返回 insufficient evidence。
...
2. **详细逻辑**
   - 与已有功能的交互关系：scope=judgement 用指定 judgement 的全部 evidence_ids（P0，不做 FTS 门槛）；scope=evidence 用单条 evidence（P0）；scope=global 用全量 evidence_pool 检索召回（P1）
   - 新增逻辑：
...
- Given 用户针对某条 judgement 提交泛化追问（如“多少标的涨了/跌了”），When 该 judgement evidence 非空，Then 后端直接将该 judgement 全量 evidence 脱敏后提供给 LLM，不因 FTS 无关键词命中而返回 insufficient_evidence
